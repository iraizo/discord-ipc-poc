...
fn main() {
    ...
    let (mut socket, response) =
        client::connect_with_config(request, config, 0).expect("Cannot connect to socket");
    ...
    loop {
        ...
        if json["cmd"] == "DISPATCH"
            && json["data"]["type"] == "DISPATCH"
            && json["data"]["pid"] == 4
        {
            println!(
                "Got payload packet, reading out, size: {:?}kb.",
                msg.len() / 1024
            );
            ...
            packet_numbers = packet_numbers + 1
        }
    }
}
